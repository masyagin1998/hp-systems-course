// gcc -std=c11 -O2 -Wall -Wextra -o priority_queue priority_queue.c -lrt
// ./priority_queue
// strace -c -e write ./priority_queue
...
void run_server() {
    ...
    printf("%d server sleeping 6 seconds...\n", getpid());
    sleep(6);

    char buffer[BUF_SIZE];
    unsigned int prio;
    for (int i = 0; i < ITERATIONS; ++i) {
        ssize_t n = mq_receive(c2s, buffer, BUF_SIZE, &prio);
        if (n <= 0) {
            break;
        }
        buffer[n] = '\0';
        printf("%d got message \"%s\" (prio=%u)\n", getpid(), buffer, prio);
    }
...
void run_client() {
    ...
    for (int i = 0; i < ITERATIONS; ++i) {
        ...
        printf("%d sending message with prio %d...\n", getpid(), i);
        mq_send(c2s, buffer, strlen(buffer), i);
        sleep(1);
    }
